...
class IniFileParser:
    ...
    @classmethod
    def _getNumber(cls, value: str) -> float | int | str:
    ...
    def load(cls, fp: IO[str] | IO[bytes]) -> dict:
        ...
        for i, line in enumerate(file_content):
            ...
            if found_section is None:
                ...
                if current_section is not None:
                    kv_list.append(dict(zip(keys, values, strict=False)))
                    keys.clear()
                    values.clear()
                # Sectionless key/value pairs
                else:
                    config |= dict(zip(keys, values, strict=False))
                    keys.clear()
                    values.clear()
        ...
        config |= dict(zip(sections, kv_list, strict=False))
        return config
