...
class InfoBar(QFrame):
    ...
    def __init__(
        ...
        self.textLayout = (
            QHBoxLayout() if self.orient == Qt.Orientation.Horizontal else QVBoxLayout()
        )
        self.widgetLayout = (
            QHBoxLayout() if self.orient == Qt.Orientation.Horizontal else QVBoxLayout()
        )
    ...
    def _initLayout(self):
        ...
        self.hBoxLayout.addWidget(
            self.iconWidget, 0, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft
        )
        ...
        self.hBoxLayout.addWidget(
            self.closeButton, 0, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft
        )
...
class InfoBarManager(QObject):
    ...
    managers: dict[InfoBarPosition, Self] = {}

    def __init__(self):
        super().__init__()
        self.spacing = 16
        self.margin = 24
        self.infoBars: weakref.WeakKeyDictionary[QWidget, dict[str, list[InfoBar]]] = (
            weakref.WeakKeyDictionary()
        )
        self.aniGroups: weakref.WeakKeyDictionary[QWidget, QParallelAnimationGroup] = (
            weakref.WeakKeyDictionary()
        )
        self.slideAnis: list[QPropertyAnimation] = []
        self.dropAnis: list[QPropertyAnimation] = []
    ...
    def remove(self, infoBar: InfoBar):
        ...
        dropAni: QPropertyAnimation | None = infoBar.property("dropAni")
        if dropAni:
            self.aniGroups[p].removeAnimation(dropAni)
            self.dropAnis.remove(dropAni)

        # remove slider animation
        slideAni: QPropertyAnimation | None = infoBar.property("slideAni")
        if slideAni:
    ...
    def _updateDropAni(self, parent: QWidget):
        for bars in self.infoBars[parent].values():
            for bar in bars:
                ani: QPropertyAnimation | None = bar.property("dropAni")
                if not ani:
                    continue
    ...
    @classmethod
    def make(cls, position: InfoBarPosition) -> Self:
...
class TopInfoBarManager(InfoBarManager):
    ...
    def _pos(self, infoBar: InfoBar, parentSize: QSize | None = None):
        p: QWidget | None = infoBar.parent()
