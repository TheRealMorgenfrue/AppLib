...
from app.components.settingcards.card_base import DisableWrapper
...
class UIGrouping():
    ...
    def _ensure_bool_child(cls, parent: AnyParentCard, child: AnyCard, group: Group):
        ...
        if isinstance(parent_option, AnyBoolSetting) and isinstance(child_option, AnyBoolSetting):
            return True
        else:
            logger.warning(f"UI Group '{group.getGroupName()}': "
    ...
    @classmethod
    def _desync_true_children(cls, wrapper: DisableWrapper | bool, child: AnyCard) -> None:
    ...
    @classmethod
    def _desync_false_children(cls, wrapper: DisableWrapper | bool, child: AnyCard) -> None:
        # Input ? Result == !Input : pass
        if isinstance(wrapper, DisableWrapper) and wrapper.isDisabled or wrapper:
            ...
            if UIGroups.DISABLE_CHILDREN in uiGroupParent:
                for child in group.getChildCards():
                    if UIGroups.SYNC_CHILDREN in uiGroupParent:
                        is_disabled = True
                        parent.getDisableChildrenSignal().connect(lambda wrapper, child=child: cls._sync_children(wrapper, child))

                    if UIGroups.DESYNC_CHILDREN in uiGroupParent:
                        is_disabled = True
                        parent.getDisableChildrenSignal().connect(lambda wrapper, child=child: cls._desync_children(wrapper, child))

                    if UIGroups.DESYNC_TRUE_CHILDREN in uiGroupParent:
                        is_disabled = True
                        parent.getDisableChildrenSignal().connect(lambda wrapper, child=child: cls._desync_true_children(wrapper, child))

                    if UIGroups.DESYNC_FALSE_CHILDREN in uiGroupParent:
                        is_disabled = True
                        parent.getDisableChildrenSignal().connect(lambda wrapper, child=child: cls._desync_false_children(wrapper, child))

                    if not is_disabled:
                        parent.getDisableChildrenSignal().connect(child.getDisableSignal().emit)

            if not is_disabled:
                if UIGroups.SYNC_CHILDREN in uiGroupParent:
                    for child in group.getChildCards():
                        if cls._ensure_bool_child(parent, child, group):
                            parent_option.getCheckedSignal().connect(lambda checked, child=child: cls._sync_children(checked, child))

                if UIGroups.DESYNC_CHILDREN in uiGroupParent:
                    for child in group.getChildCards():

                        if cls._ensure_bool_child(parent, child, group):
                            parent_option.getCheckedSignal().connect(lambda checked, child=child: cls._desync_children(checked, child))

                if UIGroups.DESYNC_TRUE_CHILDREN in uiGroupParent:
                    for child in group.getChildCards():
                        if cls._ensure_bool_child(parent, child, group):
                            parent_option.getCheckedSignal().connect(lambda checked, child=child: cls._desync_true_children(checked, child))

                if UIGroups.DESYNC_FALSE_CHILDREN in uiGroupParent:
                    for child in group.getChildCards():
                        if cls._ensure_bool_child(parent, child, group):
                            parent_option.getCheckedSignal().connect(lambda checked, child=child: cls._desync_false_children(checked, child))
...
def updateCardGrouping(setting: str, card_group: AnyCardGroup,
    ...
    if groups:
        for group in groups:
            if setting == group.getParentName():
                ...
                group.setParentCardGroup(card_group) # Instead, save a reference to the card group
            else:
                uiGroups = group.getUIGroupParent()
                if UIGroups.NESTED_CHILDREN in uiGroups or UIGroups.CLUSTERED in uiGroups:
...
def inferType(setting: str, options: dict, config_name: str) -> UITypes | None:
    ...
    if "ui_type" in options:
        ...
        card_type = UITypes.LINE_EDIT  # TODO: ui_invalidmsg should apply to all free-form input
        ...
        logger.warning(f"Config '{config_name}': Failed to infer ui_type for setting '{setting}'. "
...
def parseUnit(setting: str, options: dict, config_name: str) -> str | None:
    ...
    if "ui_unit" in options:
        ...
        if baseunit not in AppArgs.config_units.keys():
            logger.warning(f"Config '{config_name}': Setting '{setting}' has invalid unit '{baseunit}'. "
