...
class BinaryTrie(BaseSet):
    ...
    def add(self, x: _ConvertibleToInt) -> bool:
        ...
        v = u.parent
        while v is not None:
            if (v.left is None and (v.jump is None or int(v.jump.x) > ix)) or (
                v.right is None and (v.jump is None or int(v.jump.x) < ix)
            ):
                v.jump = u
            v = v.parent
        self.n += 1
